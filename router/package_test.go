...
func TestGoGetTemplateDataSource(t *testing.T) {
	...
	getConfig().dev = true
	ds := NewGoGetTemplateDataSource("skeswa", "gophr", false, common.SemverSelector{}, "", false, common.SemverCandidate{})
	...
	getConfig().dev = false
	ds = NewGoGetTemplateDataSource("skeswa", "gophr", true, selector, "/test", false, common.SemverCandidate{})
	assert.Equal(t, "github.com/skeswa/gophr", ds.GithubRoot, "Github root should be correct")
	assert.Equal(t, "master", ds.GithubTree, "Github tree should be correct")
