...
func cycleModule(m *module, gophrRoot string, env environment) error {
	...
	for i := len(m.k8sfiles) - 1; i >= 0; i-- {
		...
		if err := deleteInK8S(filepath.Join(gophrRoot, fmt.Sprintf("%s.%s.yml", k8sfile, env))); err != nil {
			return err
		}
	}
	...
	for _, k8sfile := range m.k8sfiles {
		if err := createInK8S(filepath.Join(gophrRoot, fmt.Sprintf("%s.%s.yml", k8sfile, env))); err != nil {
			return err
		}
	}
