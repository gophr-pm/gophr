...
func (m *MockRequestService) FetchCommitSHA(author string, repo string, timestamp time.Time) (string, error) {
	args := m.Called(author, repo, timestamp)
	return args.String(0), args.Error(1)
}

// FetchCommitTimestamp mocks MockRequestService.FetchCommitTimestamp.
func (m *MockRequestService) FetchCommitTimestamp(author string, repo string, sha string) (time.Time, error) {
